...
#' npm package bundled by \code{mafintosh} based on the marvelous 
#' \code{hash-index} npm package written by \code{watson}.
#' 
#' @seealso \url{https://github.com/mafintosh/hash-to-port} 
#' \url{https://github.com/watson/hash-index}
#' 
#' @export
hashToPort <- function(hash) {
  stopifnot(length(hash) == 1L, 
            is.character(hash) || is.numeric(hash) && hash %% 1L == 0L)
  ct <- V8::v8()
  ct$source(file.path(.libPaths()[1], 'hashToPortR', 'bundle.js'))
  return(ct$call('hashToPort', hash))
}